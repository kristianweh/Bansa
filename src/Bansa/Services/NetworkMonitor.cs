...
public sealed class NetworkMonitor : IDisposable
{
    ...
    // Local-address rules live in IpClassifier — shared with MainViewModel's per-row
    // IsLocalOnly classification so the ETW filter and the UI always agree.
    private static bool IsLocalIP(System.Net.IPAddress addr) => IpClassifier.IsLocal(addr);
    ...
    public void Stop()
    {
        ...
        // Session.Stop() makes Source.Process() return; give the processing task a moment
        // to observe that so it isn't abandoned mid-callback, then release the CTS.
