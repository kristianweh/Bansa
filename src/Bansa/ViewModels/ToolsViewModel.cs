...
public sealed class ToolsViewModel : IDisposable
{
    ...
    {
        const string zipName = "hwi_848.zip";
        const string url     = "https://www.hwinfo.com/files/hwi_848.zip";
        const string referer = "https://www.hwinfo.com/download/";

        var zipDir  = Path.GetDirectoryName(installDir)!;   // Data\Tools
        Directory.CreateDirectory(zipDir);
        Directory.CreateDirectory(installDir);

        // Accept any HWiNFO zip the user may have placed in Data\Tools
        var zipPath = Directory.EnumerateFiles(zipDir, "hwi*.zip").FirstOrDefault()
                   ?? Path.Combine(zipDir, zipName);

        if (!File.Exists(zipPath))
        {
            onStatus($"Downloading {zipName}…");
            onProgress(5);

            // Attempt 1 — managed HttpClient with browser UA + Referer
            bool ok = false;
            try
            {
                await DownloadFileAsync(url, zipPath, onProgress, ct,
                    referer: referer, useBrowserClient: true);
                ok = File.Exists(zipPath) && new FileInfo(zipPath).Length > 0;
            }
            catch (OperationCanceledException) { throw; }
            catch { /* fall through to BITS */ }

            // Attempt 2 — BITS (Windows HTTP stack, different server fingerprint)
            if (!ok)
            {
                onStatus("Trying BITS download…");
                onProgress(15);
                try { if (File.Exists(zipPath)) File.Delete(zipPath); } catch { }

                var script =
                    $"Start-BitsTransfer -Source '{url}' " +
                    $"-Destination '{zipPath.Replace("'", "''")}' " +
                    $"-Description 'Bansa HWiNFO'";
                ...
                ok = proc.ExitCode == 0 && File.Exists(zipPath) && new FileInfo(zipPath).Length > 0;
            }

            if (!ok)
            {
                string rel;
                try { rel = Path.GetRelativePath(AppContext.BaseDirectory, zipDir); }
                catch { rel = zipDir; }
                throw new InvalidOperationException(
                    $"Download failed. Place any HWiNFO zip (hwi*.zip) in {rel}");
            }
        }
        else
        {
            onStatus($"Found {Path.GetFileName(zipPath)}, extracting…");
            onProgress(50);
        }
