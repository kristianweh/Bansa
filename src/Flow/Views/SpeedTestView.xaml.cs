...
public partial class SpeedTestView : UserControl
{
    ...
    private async void OnStartSelfTest(object sender, RoutedEventArgs e)
    {
        ...
        {
            MessageBox.Show($"Speed test failed: {ex.Message}", "Flow", MessageBoxButton.OK, MessageBoxImage.Warning);
        }
        finally
        {
            _cts?.Dispose();
    ...
    private void OnCopyUrl(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            MessageBox.Show("Copied https://fast.com to the clipboard.", "Flow", MessageBoxButton.OK, MessageBoxImage.Information);
        }
        catch { }
    }
}
