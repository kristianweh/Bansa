...
/// <summary>
/// Manages a Windows Task Scheduler entry that launches Flow on user logon with
/// highest privileges (required because Flow needs administrator rights for ETW).
///
...
{
    private const string TaskName = "Flow_BandwidthMonitor_AutoStart";
    ...
    public static async Task<bool> SetAsync(bool enable)
    {
        try
        {
            string exePath = Process.GetCurrentProcess().MainModule?.FileName
                             ?? Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Flow.exe");
    ...
    /// <summary>Returns true if the Flow auto-start task currently exists.</summary>
